...
export default function Catalog() {
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mt-1 mb-8 max-w-prose">Every view state of the app, rendered live at a real phone viewport (390 × 844, iPhone 14/15) and scaled to fit — so proportions are exact and this never goes stale. Refer to them by number: “V2 needs…”.</p>
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
        {VIEWS.map((v) => (
          <figure key={v.id}>
            {v.src.endsWith("error-demo")
              ? <PhoneFrame title={`${v.id} ${v.name}`}><ErrorDemo /></PhoneFrame>
              : <PhoneFrame src={v.src} title={`${v.id} ${v.name}`} />}
...
function ErrorDemo() {
  return (
    <div className="w-[390px] h-[844px] flex items-center justify-center px-6 text-gray-100 text-center">
      <div className="max-w-xs w-full space-y-4">
        <div className="text-2xl font-light">Something broke</div>
        <p className="text-sm text-gray-400">Usually a new version arriving mid-load. Reloading fixes it; your progress is saved.</p>
        <div className="h-12 rounded-2xl bg-gray-100 text-black flex items-center justify-center">Reload</div>
      </div>
    </div>
  );
}
