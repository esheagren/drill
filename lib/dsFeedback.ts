...
import { formatDecision, formatReviewNote, parseDecision, parseReview } from "./designReview";
...
export function formatFeedback(rows: NoteRow[], scope: string): string {
  ...
  const live = rows.filter((r) => r.text.trim() && !/^(↕ |★ |status |order |pick |preview )/.test(r.page));
  const extras = [
  ...
  return `${head}\n\n` + live.map((r) => {
    const note = r.page.startsWith("review ") ? parseReview(r) : null;
    const decision = r.page.startsWith("decision ") ? parseDecision(r.text) : null;
    return `## ${r.page}\n${note ? formatReviewNote(note) : decision ? formatDecision(decision) : r.text.trim()}`;
  }).join("\n\n") + (extras ? `\n\n${extras}` : "");
}
