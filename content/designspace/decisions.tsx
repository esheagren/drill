...
const SUMMARY: Option[] = [
  { id: "current", name: "Again on top, then the count", note: "Again with a length picker and the two places to look at the top; answered, correct %, per-skill tally beneath — what is live (9/8)", current: true, layer: <div className="absolute inset-0 flex flex-col"><div className="px-6 pt-8 space-y-2"><div className="flex gap-2"><div className="flex-1 h-14 rounded-2xl bg-gray-100 text-black text-[18px] flex items-center justify-center">Again</div>{["2m", "4m", "8m", "12m"].map((m) => <div key={m} className={`w-12 h-14 rounded-2xl border text-[14px] tabular-nums flex items-center justify-center ${m === "8m" ? "border-gray-100 text-gray-100" : "border-gray-800 text-gray-500"}`}>{m}</div>)}</div><div className="flex justify-between text-[14px] text-gray-500 px-1 h-12 items-center"><span>skills</span><span>history</span></div></div><div className="flex-1 flex flex-col items-center justify-center px-6 pb-12 text-center"><div className="text-[72px] font-light tabular-nums leading-none">61</div><div className="text-[14px] text-gray-500 mt-2">answered · mixed · 8 min</div><div className="text-[24px] font-light mt-6 tabular-nums">55 correct · 90%</div><ul className="mt-8 w-full max-w-[320px] text-[14px] space-y-1 text-gray-500">{[["Times tables 13–20", "12/14"], ["Percent of a number", "9/10"], ["Squares to 25", "8/8"], ["Scientific notation", "7/9"]].map(([k, v]) => <li key={k} className="flex justify-between"><span>{k}</span><span className="tabular-nums">{v}</span></li>)}</ul></div></div> },
